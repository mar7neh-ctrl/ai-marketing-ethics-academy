...
def money_celebration():
    ...
    for index in range(28):
        ...
        pieces.append(
            f"<span style='--x:{x}%;--delay:{delay:.2f}s;--size:{size:.2f}rem;--drift:{drift}px'>{emoji}</span>"
        )
    st.markdown("<div class='money-rain'>" + "".join(pieces) + "</div>", unsafe_allow_html=True)


def mascot_welcome(message="Hi! I’ll be your AI governance guide."):
    st.markdown(
        "<div class='mascot-stage'><span class='mascot-robot'>🤖</span>"
        "<span class='mascot-hand'>👋</span></div>"
        f"<div class='mascot-message'>{message}</div>",
        unsafe_allow_html=True,
    )
...
def completion_celebration():
    ...
    for index in range(42):
        ...
        size = 1.4 + (index % 6) * .20
        pieces.append(
            f"<span class='confetti-piece' style='--x:{x}%;--delay:{delay:.2f}s;--size:{size:.2f}rem'>"
            f"{emojis[index % len(emojis)]}</span>"
        )
    st.markdown(
        "<div class='celebration-layer'>" + "".join(pieces)
        + "<div class='celebration-mascot'>🤖🏆</div></div>",
        unsafe_allow_html=True,
    )
